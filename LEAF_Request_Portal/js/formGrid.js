...
var LeafFormGrid = function (containerID, options) {
  ...
  function setHeaders(headersIn) {
    ...
    if (showIndex) {
      ...
      temp +=
        `<th scope="col" tabindex="0" id="${prefixID}header_UID" style="text-align: center" aria-label="sort by unique ID">UID` +
          ...
          `<button type="button" tabindex="-1" aria-label="sortable" class="btn_formgrid_sort" ${btnStyles}></button>
        </th>`;
    }
    ...
    for (let i in headers) {
      ...
      const textName = scrubHTML(headers[i]?.name || '');
      const ariaLabel = textName.replace(/['"]+/gu, ''); //strip out icons and chars that could truncate the html
      ...
      domThead.insertAdjacentHTML('beforeend',
        ...
        `${textName}<span id="${prefixID}header_${headers[i].indicatorID}_sort" class="${prefixID}sort"></span>
        <button type="button" tabindex="-1" aria-label="sortable" class="btn_formgrid_sort" ${btnStyles}></button>
        </th>`);
